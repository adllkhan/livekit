...
func TestNewConfigKeepsConfiguredNodeIP(t *testing.T) {
	const content = `rtc:
  ...
  node_ip: 10.1.6.19`
	conf, err := NewConfig(content, true, nil, nil)
	require.NoError(t, err)
	require.Equal(t, "10.1.6.19", conf.RTC.NodeIP.V4)
	require.False(t, conf.RTC.NodeIPAutoGenerated)
	// external IP discovery for ICE candidates stays enabled
	require.True(t, conf.RTC.UseExternalIP)
	require.True(t, conf.RTC.AdvertiseInternalIP)
}

func TestNewConfigNodeIPs(t *testing.T) {
	t.Run("addresses are advertised and the first one becomes the node identity", func(t *testing.T) {
		const content = `rtc:
  use_external_ip: true
  node_ips:
    - 89.218.81.183
    - 10.1.6.19`
		conf, err := NewConfig(content, true, nil, nil)
		require.NoError(t, err)
		require.Equal(t, []string{"89.218.81.183", "10.1.6.19"}, conf.RTC.NodeIPs)
		// single address for Redis registration and inter-node communication
		require.Equal(t, "89.218.81.183", conf.RTC.NodeIP.V4)
		require.Equal(t, "89.218.81.183", conf.RTC.NodeIP.PrimaryIP())
		require.False(t, conf.RTC.NodeIPAutoGenerated)
	})

	t.Run("node_ip keeps being the node identity when both are set", func(t *testing.T) {
		const content = `rtc:
  use_external_ip: true
  node_ip: 10.1.99.215
  node_ips:
    ...
    - 10.1.6.19`
		...
		require.NoError(t, err)
		require.Equal(t, []string{"89.218.81.183", "10.1.6.19"}, conf.RTC.NodeIPs)
		require.Equal(t, "10.1.99.215", conf.RTC.NodeIP.PrimaryIP())
	})

	t.Run("both families are picked up for the node identity", func(t *testing.T) {
		const content = `rtc:
  node_ips:
    - 2001:db8::1
    - 89.218.81.183
    - 10.1.6.19`
		...
		require.NoError(t, err)
		require.Equal(t, "89.218.81.183", conf.RTC.NodeIP.V4)
		require.Equal(t, "2001:db8::1", conf.RTC.NodeIP.V6)
	})

	t.Run("duplicates are dropped, order is kept", func(t *testing.T) {
		const content = `rtc:
  node_ips:
    - 89.218.81.183
    - 10.1.6.19
  ...
  node_ips:
    ...
    - not-an-ip`
		_, err := NewConfig(content, true, nil, nil)
		require.ErrorContains(t, err, `invalid node_ips entry "not-an-ip"`)
	})

	t.Run("unset leaves node_ip behaviour untouched", func(t *testing.T) {
		const content = `rtc:
  node_ip: 10.1.6.19`
		conf, err := NewConfig(content, true, nil, nil)
		require.NoError(t, err)
		require.Empty(t, conf.RTC.NodeIPs)
		require.Equal(t, "10.1.6.19", conf.RTC.NodeIP.PrimaryIP())
	})
}
